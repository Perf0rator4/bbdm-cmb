...
Почему денойзеру нужен Planck отдельным каналом. Этот шаг точен, только если
`pred = E[y | x_t, x0]`. Для коэффициентов верно `c_xt + c_yt = 1`, и
Planck-составляющая состояния остаётся на мосту (`a_t = m_t`) лишь тогда,
когда в `pred` нет Planck-составляющей. Сеть, видящая только `x_t`, учит
`E[y | x_t]` и не может отделить `m_t * x0` от `y`, поэтому Planck — в
полосе 0.1–0.3 это почти чистый приборный шум Planck, в 6.3 раза мощнее
цели — протекает на выход. Прогон через точные коэффициенты даёт такой
остаток: сеть-тождество — 1.000 от Planck (run 4 измерил TF 6.30 при
Planck/цель = 6.29), точное `E[y | x_t]` — 0.30 (0.57 мощности цели
некоррелированного шума), точное `E[y | x_t, x0]` — 0.000.
"""
...
class BBDM(nn.Module):
    ...
    Args:
        model: сеть, предсказывающая чистый y по (x_t, t) или, если у неё
            `cond_ch > 0`, по (x_t, t, x0). Все вызовы сети идут через
            `denoise()`, которое выбирает сигнатуру.
    ...
    def denoise(self, x_t, t, cond):
        """Предсказание чистого y. Единственная точка вызова сети.

        `cond` -- ЧИСТЫЙ патч Planck (x0 в обучении, y_cond в сэмплере).
        Передаётся сети, только если она построена с `cond_ch > 0`; сети
        run 3/4 и тестовые oracle-модели вызываются как раньше, `(x_t, t)`.
        Лосс, сэмплер и все диагностики обязаны идти через этот метод, иначе
        условие легко забыть в одном из мест и получить модель, которая на
        обучении видит Planck, а на инференсе -- нет.
        """
        if getattr(self.model, "cond_ch", 0) > 0:
    ...
    def sample(self, planck, S=200, generator=None, progress=False):
        ...
        for i in iterator:
            ...
            pred = self.denoise(x_t, t, y_cond)
